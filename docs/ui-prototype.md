...
担当者の日常導線（`Home -> Execute review -> Approve or Comment`）は、出先や会議室から即応したい場面が多いため、スマートフォン表示にも対応します。管理者向けの画面（提案レビュー、学習状況、昇格確認）は意思決定に情報密度が必要なため、モバイルでは**閲覧専用**とし、判断操作はデスクトップに誘導します。

### レスポンシブ方針

| ブレークポイント | 想定デバイス | レイアウト方針 |
| --- | --- | --- |
| `>= 1024px` | PC / タブレット横 | 既存の 2 カラムレイアウトをそのまま利用 |
| `768px - 1023px` | タブレット縦 | 2 カラムを縦積みに変換、操作ボタンは下部固定 |
| `< 768px` | スマートフォン | 1 カラム縦積み、主要アクションのみ表示、詳細は折りたたみ |

| 原則 | モバイルでの扱い |
| --- | --- |
| 親指操作で完結 | 主要ボタンを画面下部に固定し、上部に情報、下部にアクションを配置 |
| 情報の取捨選択 | 詳細情報（過去実績など）はデフォルトで折りたたみ、必要時に展開 |
| 縦積みレイアウト | 左右 2 カラムの比較画面は「上＝画面、下＝判断」の縦並びに変換 |
| 判断は同じ | 表示は簡略化しても、承認 / 差し戻しの判断基準と記録内容は PC 版と同一 |
...
| 領域 | モバイルでの工夫 |
| --- | --- |
| `KPI` | 横並び 4 項目を縦 1 列に変換、running は省略可 |
| `Pending` | 各案件を 2 行表示（ID + ステップ / 時刻）にして親指でタップしやすくする |
| `Running` | デフォルトで折りたたみ、`show` で展開 |
...
| 領域 | モバイルでの工夫 |
| --- | --- |
| `step` | 横長のプログレスバーを簡略記号に変換、「何 / 何」の数字で現在位置を補完 |
| `Checks passed` | 項目を縦積み、アイコン `+` でチェック完了を示す |
| `Show detail` | 詳細表示は折りたたみ、必要時のみ展開 |
| `fixed bottom bar` | `Approve next` と `Send back` を画面下部に固定配置 |
...
| 領域 | モバイルでの工夫 |
| --- | --- |
| `ALERT` | 最上部に赤帯で配置し、差分値を大きく表示 |
| `Active screen` | スクリーンショットはサムネイル表示、タップで全画面拡大 |
| `Values` | 左右比較を縦並びに変換、ラベル幅を揃える |
| 下部ボタン | 3 択を等幅で並べ、誤タップしにくい大きさを確保 |
...
| 領域 | モバイルでの工夫 |
| --- | --- |
| `Your comment` | テキストエリアを広く取り、音声入力ボタンを併設して移動中でも入力可能にする |
| `Similar note` | 候補を 1 件に絞って表示（PC 版より短縮）、`Quote` で即流用 |
| `Examples` | モバイルでは省略（PC 版では表示） |
| `Submit` | 送信後はホーム画面に戻る導線を確保 |

### モバイルでは行わない操作

以下の操作は、**情報密度と判断の慎重さが求められるため、モバイルでは閲覧のみ**とし、実行時は PC へ誘導します。

| 操作 | モバイルでの挙動 |
| --- | --- |
| 変更提案の承認 / 却下 | 提案内容は閲覧可、採否ボタンは無効化して「PC で判断してください」と表示 |
| 信頼レベル昇格 | 昇格候補の確認は可、`Approve upgrade` は PC でのみ有効 |
| 手順定義の初回承認 | マニュアル原文との比較が必要なため、モバイルでは非表示 |

この方針により、「現場の即応性」と「重要判断の慎重さ」を両立させます。
