        ...
        <div className="flex items-start justify-between gap-2 flex-wrap">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              <span
                className={`text-[11px] font-semibold px-1.5 py-0.5 rounded border ${changeTypeColors[proposal.changeType]}`}
              >
                {changeTypeLabels[proposal.changeType]}
              </span>
            </div>
            <p className="text-xs text-muted-foreground truncate">
              対象: {proposal.targetProcedureJp}
            </p>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-md border bg-muted/30 p-3 space-y-1.5">
          ...
          {proposal.diff.before.map((line, i) => (
            <div
              key={`before-${i}`}
              className="text-xs font-mono bg-red-50 text-red-700 px-2 py-1 rounded border border-red-200 line-through break-words"
            >
              − {line}
            </div>
          ))}
          {proposal.diff.after.map((line, i) => (
            <div
              key={`after-${i}`}
              className="text-xs font-mono bg-emerald-50 text-emerald-700 px-2 py-1 rounded border border-emerald-200 break-words"
            >
              + {line}
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="p-3 rounded-md bg-amber-50/50 border border-amber-200">
            ...
            <p className="text-sm">
              ...
              <Num className="font-semibold">
                {proposal.impactAnalysis.affectedRunsLast30d}
              </Num>
              件 に影響
            </p>
          ...
          <div className="p-3 rounded-md bg-emerald-50/50 border border-emerald-200">
            ...
            <p className="text-sm">
              過去{" "}
              <Num className="font-semibold">
                {proposal.impactAnalysis.preventableMissesCount}
              </Num>
              件 の差し戻しを防止可能
            </p>
            <p className="text-xs text-muted-foreground mt-1">
        ...
        <div>
          ...
          <div className="space-y-1.5 pl-1">
            {proposal.voices.map((v, i) => (
              <div
                key={i}
                className="text-sm flex flex-wrap sm:flex-nowrap gap-x-2 gap-y-1 py-1 px-2 rounded bg-muted/30"
              >
                ...
                <span className="text-muted-foreground text-xs min-w-0 break-words basis-full sm:basis-auto">
                  {v.text}
                </span>
              </div>
            ))}
          ...
          <>
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              <Button
                variant="secondary"
                size="tap"
                onClick={() => onStatusChange(proposal.id, "held")}
              >
                保留
              </Button>
            </div>
