...
const dotStyleByCategory: Record<
  ErrorCategory,
...
const routingChipByOutcome: Record<
  RecentCorrection["routingOutcome"],
  { tone: StatusTone; label: string }
> = {
  tier2: { tone: "violet", label: "手順の変更提案を作成" },
  tier1: { tone: "teal", label: "ナレッジに追加" },
  log_only: { tone: "neutral", label: "記録のみ" },
};

export default function RecentCorrectionsTimeline({
  corrections,
}: RecentCorrectionsTimelineProps) {
  return (
    <Card className="relative">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <h2 className="text-base font-semibold leading-[1.4]">
              最近の修正事項
            </h2>
          </div>
        </div>
