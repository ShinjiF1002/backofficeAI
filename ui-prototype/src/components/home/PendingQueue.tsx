...
import { Card, CardContent, CardHeader, FadeIn } from "crystalline-ui";
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            />
            <h2 className="text-base font-semibold leading-[1.4] truncate">
              承認待ちキュー
            </h2>
          </div>
          {count > 0 && (
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          AI の実行結果を確認し、承認または差し戻しを行ってください
        </p>
      </CardHeader>
      <CardContent>
        {count === 0 ? (
          ...
          <div className="space-y-2">
            {tasks.map((task, i) => {
              ...
              const hasFailedCheck =
                currentStep?.checks.some((c) => c.status === "ng") ?? false;
              const primaryData = task.keyData[0];
              const secondaryData = task.keyData[1];
              return (
                <FadeIn key={task.id} index={i}>
                  <ListRowCard
                    leading={
                    ...
                    subtitle={
                      <span className="flex flex-wrap gap-x-1.5 gap-y-0 items-baseline min-w-0">
                        {primaryData?.value && (
                          <span className="truncate">{primaryData.value}</span>
                        )}
                        {secondaryData?.value && (
                          <>
                            <span
                    ...
                    trailing={
                      <ConfidenceBadge confidence={task.confidence} size="sm" />
                    }
