...
import { Card, CardContent, CardHeader, CardTitle, Num } from "crystalline-ui";
...
export default function CostTrendCard({ metrics }: CostTrendCardProps) {
  ...
  const savingsGrowth = prev
    ? ((latest.netBenefitJpy - prev.netBenefitJpy) / prev.netBenefitJpy) * 100
    : 0;
            ...
            >
              ...
              <div className="mt-1.5 space-y-1">
                <div className="flex justify-between text-xs">
                  ...
                  <Num className="font-semibold text-emerald-600">
                    ¥{(m.estimatedSavingsJpy / 1000).toFixed(0)}K
                  </Num>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-muted-foreground">API費</span>
                  <Num className="font-semibold text-amber-600">
                    ¥{(m.totalApiCostJpy / 1000).toFixed(0)}K
                  </Num>
                </div>
                <div className="flex justify-between text-xs pt-1 border-t border-border/60">
                  <span className="text-muted-foreground">ネット</span>
                  <Num className="font-semibold text-primary">
                    ¥{(m.netBenefitJpy / 1000).toFixed(0)}K
                  </Num>
                </div>
              </div>
            </div>
        ...
        {prev && (
          <div className="mt-3 flex items-center gap-2 text-xs">
            ...
            <span className="text-muted-foreground">
              前月比ネット効果:{" "}
              <Num className="text-emerald-600 font-semibold">
