...
const statusMeta: Record<string, { label: string; tone: StatusTone }> = {
  ...
  sent_back: { label: "差し戻し", tone: "warning" },
  failed: { label: "失敗", tone: "danger" },
  in_progress: { label: "実行中", tone: "info" },
};
...
export default function RunHistoryPage() {
  ...
  const inProgress = runHistory.filter(
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <p className="text-xs text-muted-foreground leading-[1.4]">
              各実行には、画面キャプチャ・実行ログ・承認記録・修正コメントが紐づいています
            </p>
          </CardHeader>
          <CardContent>
                  ...
                  <Card key={run.id} variant="default" size="sm">
                    <CardContent className="p-3">
                      <div className="flex items-start gap-3">
                        <CategoryIcon
                          category={run.procedureCategory}
                          size="sm"
                        />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2 flex-wrap">
                            <div className="flex items-center gap-2 min-w-0 flex-wrap">
                              ...
                              <Num className="text-[11px] text-muted-foreground font-num truncate">
                                {run.id}
                              </Num>
                              <StatusPill tone={st.tone}>{st.label}</StatusPill>
                            </div>
                            <Num className="text-[11px] text-muted-foreground font-num tabular-nums shrink-0">
                              {run.startedAt}
                            </Num>
                          </div>
                          <div className="flex items-center gap-2 sm:gap-3 mt-2 text-[10px] sm:text-[11px] text-muted-foreground flex-wrap">
                            ...
                            <span className="flex items-center gap-1 shrink-0 tabular-nums">
                              ...
                              <Num>{run.humanApprovals}</Num>/
                              <Num>{run.stepCount}</Num> 承認
                            </span>
                            {run.hasScreenshots && (
