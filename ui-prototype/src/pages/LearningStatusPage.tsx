...
export default function LearningStatusPage() {
  ...
  const driftedMobileHidden =
    driftedWorkflows.length - driftedMobileVisible.length;

  const latestWeek = learningMetrics[learningMetrics.length - 1];
  const firstWeek = learningMetrics[0];
  const approvalsReduced =
    firstWeek.humanApprovalsRequired - latestWeek.humanApprovalsRequired;
  const approvalsReducedPct = Math.round(
    (approvalsReduced / firstWeek.humanApprovalsRequired) * 100,
  );
      ...
      <FadeIn index={1}>
        {driftedWorkflows.length > 0 && (
          <Alert variant="warning">
            ...
            <AlertTitle className="font-semibold leading-[1.4]">
              UI 変更を検知した業務があります
            </AlertTitle>
              ...
              <div className="sm:hidden space-y-2">
                ...
                {driftedMobileHidden > 0 && (
                  <p className="text-xs text-muted-foreground">
                    他 <Num>{driftedMobileHidden}</Num> 件
                  </p>
                )}
              </div>
            </AlertDescription>
            ...
            {comments.map((c) => (
              ...
              >
                <div className="flex items-start gap-3 flex-wrap sm:flex-nowrap">
                  <span className="text-xs text-muted-foreground shrink-0 w-12 font-mono tabular-nums">
                    {c.date}
                  </span>
                  <span className="text-xs font-medium shrink-0 w-20 truncate">
                    {c.author}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs break-words leading-[1.4]">
                      {c.text}
                    </p>
                    {(c.before || c.after) && (
                      <div className="mt-1">
                        <DiffPill before={c.before} after={c.after} />
                      </div>
                    )}
                  </div>
