...
export default function UpgradePage() {
  ...
  const candidates = workflows.filter(
    (w) => w.recommendation.kind === "ready_to_upgrade",
  );

  const keepHuman = upgradeTarget.steps
    .filter((s) => s.next === "human")
    .map((s) => s.name);
  const autoRun = upgradeTarget.steps
    .filter((s) => s.next === "ai")
    .map((s) => s.name);
      ...
      <div className="space-y-6 max-w-4xl">
        ...
        <Alert variant="info">
          ...
          <AlertDescription className="text-sm leading-[1.4]">
            <strong>信頼レベル昇格とは:</strong>{" "}
            十分な実績を積んだ業務について、人間の承認ステップを削減し処理効率を向上させます。
        ...
        <Card className={tintClasses.amber}>
          ...
          <CardContent className="text-sm space-y-2">
            <p className="leading-[1.4]">
              昇格後も、以下の場合は自動的に全ステップ承認モードに戻ります:
            </p>
            <ul className="space-y-1 ml-4 text-xs leading-[1.5]">
              ...
              <li className="flex items-start gap-2">
                <span className="text-muted-foreground">・</span>
                <span>
                  高リスク業務（高額送金など）は、どれだけ実績を積んでも完全自律化されません
                </span>
              </li>
            </ul>
          </CardContent>
    ...
    return (
      <div className="space-y-8 max-w-4xl">
        <div className="flex flex-col items-center justify-center py-12 gap-4">
          ...
          <p className="text-lg font-medium leading-[1.4]">
            昇格を承認しました
          </p>
          <p className="text-muted-foreground text-center leading-[1.4]">
            {selectedWf.jpName} を {trustModeLabels[upgradeTarget.nextMode]}{" "}
            モードに昇格しました
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-sm">
              <Card size="sm" className={tintClasses.slate}>
                <CardContent className="py-3">
                  <p className="text-xs text-muted-foreground mb-1 leading-[1.4]">
                    変更前
                  </p>
                  <p className="font-medium leading-[1.4]">
                    {trustModeLabels[upgradeTarget.currentMode]}（
                    <Num>{upgradeTarget.currentApprovals}</Num> 回承認）
                  </p>
                </CardContent>
              </Card>
              <Card size="sm" className={tintClasses.emerald}>
                <CardContent className="py-3">
                  ...
                  <p className="font-medium text-emerald-700 leading-[1.4]">
                    {trustModeLabels[upgradeTarget.nextMode]}（
                    <Num>{upgradeTarget.nextApprovals}</Num> 回承認）
                  </p>
                </CardContent>
              </Card>
            </div>
        ...
        <Card>
          ...
          <CardContent className="text-sm text-muted-foreground space-y-2 leading-[1.4]">
            <p>
              チェックポイント承認モードでの稼働を開始しました。次回以降のタスクから適用されます。
            </p>
            <p>現在実行中のタスクには影響しません。</p>
            <p>
              精度が 90%
              を下回った場合、自動的に全ステップ承認モードに戻ります（自動降格）。
            </p>
          </CardContent>
        </Card>
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <FadeIn>
        <Heading as="h1">昇格の確認: {selectedWf.jpName}</Heading>
        <Text muted>AI の実績に基づいて、承認プロセスを効率化します</Text>
      </FadeIn>
        ...
        <CardContent>
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="text-amber-600 font-medium tabular-nums">
                降格閾値: <Num>{upgradeTarget.demotionThreshold}</Num>%
              </span>
              <span className="text-emerald-600 font-medium tabular-nums">
                現在: <Num>{upgradeTarget.currentAccuracy}</Num>%
              </span>
            </div>
            <div className="relative h-3 rounded-full bg-muted overflow-hidden">
              ...
              <div
                ...
                style={{
                  left: `calc(${upgradeTarget.currentAccuracy}% - 4px)`,
                }}
                title="現在の精度"
              />
            </div>
          ...
          <p className="text-xs text-muted-foreground mt-3 leading-[1.4]">
            ...
            <strong className="text-emerald-600 tabular-nums">
              ...
              <Num>
                {(
                  upgradeTarget.currentAccuracy -
                  upgradeTarget.demotionThreshold
                ).toFixed(1)}
              </Num>
              %
            </strong>{" "}
        ...
        <CardContent>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 lg:gap-4">
            {[
              {
                label: "承認回数",
                before: `${upgradeTarget.currentApprovals}回`,
                after: `${upgradeTarget.nextApprovals}回`,
                change: `${Math.round((1 - upgradeTarget.nextApprovals / upgradeTarget.currentApprovals) * 100)}%削減`,
              },
              {
                label: "担当者の関与時間",
                before: "約15分",
              ...
              <Card key={item.label} size="sm" className={tintClasses.slate}>
                <CardContent className="text-center py-3">
                  ...
                  <p className="text-[11px] text-emerald-600 mt-1 leading-[1.4]">
                    {item.change}
                  </p>
                </CardContent>
              </Card>
      ...
      <Alert variant="warning">
        ...
        <AlertDescription className="text-sm leading-[1.4]">
          <strong>注意:</strong> チェックポイント承認モードでは、AI
          自動実行ステップ（{autoRun.join("、")}
          ）の途中で人間が介入することはできません。 精度が 90%
          を下回った場合、自動的に全ステップ承認モードに戻ります。
        </AlertDescription>
      </Alert>

      <div className="flex flex-wrap gap-2 sm:gap-3 pt-2">
        <Button variant="primary" size="cta" onClick={approveUpgrade}>
          昇格を承認
        </Button>
        <Button
          variant="secondary"
